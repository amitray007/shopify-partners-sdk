...
class DetailedQueryBuilder:
    ...
    def get_scalar_fields(
        self, type_name: str, visited: Optional[set[str]] = None
    ) -> list[str]:
        ...
        for impl_name in implementations:
            ...
            for field in impl_fields[:10]:  # Limit to first 10 fields
                ...
                if base_type in self.type_registry:
                    ...
                    if (
                        target_type.get("kind") == "OBJECT"
                        and "Connection" not in base_type
                    ):
    ...
    def build_connection_structure(
        self, field_name: str, field_info: dict, indent: str = "    "
    ) -> list[str]:
        ...
        for arg in args:
            ...
            if "first" in arg_name:
                ...
                base_arg_type = (
                    arg_type.replace("!", "").replace("[", "").replace("]", "")
                )
                ...
                arg_parts.append(
                    f'{indent}  {arg_name}: "gid://partners/{gid_type}/123"'
                )
            elif "Id" in arg_name:
    ...
    def build_detailed_query(self, query_name: str) -> dict[str, Any]:
        ...
        if variables:
            query_lines.append(
                f"query {query_name.title()}Query({', '.join(variables)}) {{"
            )
        else:
        ...
        if return_type_info:
            if return_type_info.get("kind") == "INTERFACE":
                ...
                connection_lines = self.build_connection_structure(
    ...
    def build_detailed_mutation(self, mutation_name: str) -> dict[str, Any]:
        ...
        if variables:
            mutation_lines.append(
                f"mutation {mutation_name.title()}Mutation({', '.join(variables)}) {{"
            )
        else:
...
def main():
    ...
    for query in examples["queries"]:
        ...
        if query.get("deprecated"):
            md_lines.append(
                f"**⚠️ DEPRECATED:** {query.get('deprecation_reason', 'This field is deprecated')}"
            )
            md_lines.append("")
    ...
    print(
        f"Generated {len(examples['queries'])} queries and {len(examples['mutations'])} mutations"
    )
