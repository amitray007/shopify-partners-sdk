...
def load_schema(version_path: str) -> Dict:
    ...
    introspection_file = os.path.join(version_path, "introspection.json")
    if os.path.exists(introspection_file):
        with open(introspection_file, 'r', encoding='utf-8') as f:
            return json.load(f)
    return {}
...
def extract_types(schema: Dict) -> Dict[str, Dict]:
    ...
    types = {}
    if 'data' in schema and '__schema' in schema['data'] and 'types' in schema['data']['__schema']:
    ...
    if query_type_name:
        for type_info in schema_info['types']:
            if type_info.get('name') == query_type_name:
                operations["queries"] = [f.get('name') for f in type_info.get('fields', []) if f.get('name')]
                break
        ...
        if mutation_type_name:
            for type_info in schema_info['types']:
                if type_info.get('name') == mutation_type_name:
                    operations["mutations"] = [f.get('name') for f in type_info.get('fields', []) if f.get('name')]
                    break
...
def compare_versions():
    ...
    schema_dirs = ["../versions/2024-10", "../versions/2025-01", "../versions/2025-04", "../versions/2025-07", "../versions/unstable"]
    schemas = {}
    ...
    for i in range(len(versions) - 1):
        ...
        for type_name in common_types:
            ...
            if current_type.get('kind') == 'OBJECT' and next_type.get('kind') == 'OBJECT':
                current_fields = {f.get('name'): f for f in current_type.get('fields', []) if f.get('name')}
                next_fields = {f.get('name'): f for f in next_type.get('fields', []) if f.get('name')}
    ...
    for comparison in comparison_results:
        report_lines.append(f"## Changes from {comparison['from_version']} to {comparison['to_version']}")
        ...
        if comparison['modified_types']:
            ...
            for mod_type in comparison['modified_types']:
                ...
                if mod_type['added_fields']:
                    report_lines.append(f"  - Added fields: {', '.join(mod_type['added_fields'])}")
                if mod_type['removed_fields']:
                    report_lines.append(f"  - Removed fields: {', '.join(mod_type['removed_fields'])}")
                report_lines.append("")
