...
class GraphQLTypeAnalyzer:
    ...
    def resolve_type(self, type_def: dict[str, Any], depth: int = 0) -> dict[str, Any]:
        ...
        if kind in ["SCALAR", "ENUM", "INPUT_OBJECT"]:
            result = {"type": name, "kind": kind, "description": ""}
                ...
                if fields:
                    ...
                    for field in fields[:10]:  # Limit number of fields shown
                        ...
                        if field.get("isDeprecated"):
                            result["fields"][field["name"]]["deprecation_reason"] = (
...
class GraphQLSchemaAnalyzer:
    ...
    def analyze_field(self, field: dict[str, Any]) -> dict[str, Any]:
        ...
        field_info = {
            ...
            "deprecated": field.get("isDeprecated", False),
            "deprecation_reason": (
                field.get("deprecationReason") if field.get("isDeprecated") else None
            ),
            "arguments": [],
            "return_type": {},
        }
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        stats = {
            ...
            "query_count": (
                len(self.query_root.get("fields", [])) if self.query_root else 0
            ),
            "mutation_count": (
                len(self.mutation_root.get("fields", [])) if self.mutation_root else 0
            ),
...
def format_type_info(type_info: dict[str, Any], indent: str = "") -> str:
    ...
    if type_info.get("possible_types"):
        result.append(
            f"{indent}Possible Types: {', '.join(type_info['possible_types'])}"
        )
    ...
    if type_info.get("fields"):
        ...
        for field_name, field_info in list(type_info["fields"].items())[
            :5
        ]:  # Show first 5 fields
            result.append(
                f"{indent}  - {field_name}: {field_info.get('type', 'Unknown')}"
            )
            if field_info.get("description"):
                result.append(f"{indent}    Description: {field_info['description']}")
            if field_info.get("deprecated"):
                result.append(
                    f"{indent}    DEPRECATED: {field_info.get('deprecation_reason', 'No reason given')}"
                )

        if len(type_info["fields"]) > 5:
            result.append(
                f"{indent}  ... and {len(type_info['fields']) - 5} more fields"
            )
...
def format_field_analysis(field: dict[str, Any], field_type: str) -> str:
    ...
    lines = [f"\n{'='*60}", f"{field_type.upper()}: {field['name']}", f"{'='*60}"]

    if field["description"]:
        lines.extend(["Description:", f"  {field['description']}"])
    ...
    # Return type
    lines.extend(["\nReturn Type:", format_type_info(field["return_type"], "  ")])
...
def main():
    ...
    report_lines = [
        ...
        *[
            f"  {kind}: {count}"
            for kind, count in sorted(stats["types_by_kind"].items())
        ],
        "",
        "DETAILED ANALYSIS",
        "=" * 50,
    ...
    if analyzer.type_analyzer.connection_types:
        report_lines.extend(
            [
                ...
                *[
                    f"  • {conn_type}"
                    for conn_type in sorted(analyzer.type_analyzer.connection_types)
                ],
            ]
        )
    ...
    if analyzer.type_analyzer.enum_types:
        ...
        for enum_name, enum_values in sorted(analyzer.type_analyzer.enum_types.items()):
            report_lines.extend(
                [f"\n{enum_name}:", f"  Values: {', '.join(enum_values)}"]
            )
    ...
    try:
        ...
        print(
            f"Found {stats['query_count']} queries and {stats['mutation_count']} mutations"
        )
